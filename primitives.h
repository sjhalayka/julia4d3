...
{
public:
	inline vertex_4(void) : x(0.0f), y(0.0f), z(0.0f), w(0.0) { }
...
inline const float length(vertex_4 v)
{
	return std::sqrt(v.self_dot());
}

inline const vertex_4 normalize(vertex_4 v)
{
	float len = length(v);
...
public:
	...
	vertex_3& operator=(const vertex_3& rhs)
	{
		x = rhs.x;
		y = rhs.y;
		z = rhs.z;
...
class vertex_3_with_index : public vertex_3
{
public:
	GLuint index;
};

class vertex_3_with_normal : public vertex_3
{
public:
	float nx, ny, nz;
};
